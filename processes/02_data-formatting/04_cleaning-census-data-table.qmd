...
make_long_tables <- function(x){
  ...
  tables_to_bind <- select(x, com_tables, dioceses_tables, nations_tables)
...
coms_by_nation <- map(coms_by_nation, make_long_tables)
```

## Save data

```{r saveData}
saveRDS(coms_by_nation, here("data", "intermediate", "census", "merged-data", "02_grouped-by-level-and-nation_2022-10-25.RDS"))
```