...
#     mutate,
#     across(
#       c(com_table, dioceses_tables),
#       some_function 
#       )
#     )


# mutate(across(c(col1, col2), some_function() ))

add_com_column <- function(this_list, com_name){
...
coms_by_nation$England$dioceses_tables |> keep(function(x) any(x[[1]][["com"]] == "Greenwich"))

## southwark diocese has data for CoI church????
```
...
#   mutate,
#   com_tables =
#     list(map(com_tables, relocate, com)
#          ))
```
...
```{r rbindWarner}
rbind_warn <- function(x, y, just_warn = TRUE){
  
  tryCatch(
    expr = {rbind(x,y)},
    error = function(cnd){
      
      bad.names <- names(y)[!names(y) %in% names(x)]
      warning("Rbind unsucessful in ", unique(y[["com"]]),
...
make_long_tables <- function(x){
  ...
  tables_to_bind <- select(x, com_tables, dioceses_tables)
