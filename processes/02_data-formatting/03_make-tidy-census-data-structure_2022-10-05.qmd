...
com_tables <- lapply(com_names_list, table_read)
nation_tables <- lapply(path_nation_list, table_read)
dioceses_tables <- lapply(path_dioceses_list, table_read)

is.list(com_tables)
overall_table <- c(com_tables, nation_tables, dioceses_tables)
overall_table
saveRDS(overall_table)
```
...
nation_tables <- lapply(path_nation_list,  function(x){
      path_csvs_coms <- here("data", "inputs", "census", "nations", "csvs", x)
      coms_sub <- dir(path_csvs_coms, full.names = TRUE)
      tables <- lapply(coms_sub, read_csv) 
      return(tables)
 }
)
str(nation_tables)
...
dioceses_tables <- lapply(path_dioceses_list,  function(x){
      path_csvs_coms <- here("data", "inputs", "census", "dioceses", "csvs", x)
      coms_sub <- dir(path_csvs_coms, full.names = TRUE)
      tables <- lapply(coms_sub, read_csv) 
      return(tables)
 }
